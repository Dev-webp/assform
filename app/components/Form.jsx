...
const Form = () => {
  ...
  return (
    ...
    {currentStep === 0 ? (
      <div>
        <h3 className="text-xl font-semibold">Select a Country for Immigration Evaluation</h3>
        <div className="flex flex-wrap gap-3 mt-2">
          {Object.keys(mcqQuestions).map((country) => (
            <button
              key={country}
              onClick={() => setSelectedCountry(country)} // Just select, don't advance step
              className={`px-3 py-3 border border-slate-700 hover:bg-gray-600 h-12  
        ...
        {selectedCountry && (
          <button
            className={`px-6 py-3 h-12 w-28 mt-4 text-white flex items-center justify-center gap-2 transition duration-300 ease-in-out 
              ${selectedCountry ? 'bg-gray-800 hover:bg-gray-900' : 'bg-orange-500 hover:bg-orange-600'}`}
      ...
      <div>
        ...
        <div className="flex justify-between mt-4">
          <button 
            className="px-6 py-3 h-12 w-28 mt-4 bg-gray-500 text-white flex items-center justify-center gap-2 transition duration-300 ease-in-out hover:bg-orange-600" 
      ...
      // Handle other steps or form submission
        ...
        // User Form Step
        <form onSubmit={handleSubmit} className="w-full flex flex-col items-start md:ml-8 lg:ml-10">
         <h1 className="text-3xl font-bold text-center text-orange-500 mt-12 mb-1">
  Almost done!
</h1>
{/* <h1 className="text-3xl font-bold text-center text-black mb-8">
  ...
  <button type="submit" className="px-6 py-3 h-12 w-28 mr-12 bg-orange-500 text-white flex items-center transition duration-300 ease-in-out hover:bg-orange-600" disabled={loading}>
